...
std::cout << "Exits: ";
    ...
    std::map<std::string, Room*>::const_iterator it = exits.begin();
    for (; it != exits.end(); ++it) { // Iterate through exits map - Generated by Copilot

        std::cout << it->first;

        std::map<std::string, Room*>::const_iterator nextIt = it;
        ++nextIt;
...
void Room::removeItem(const std::string& item_name) {
    ...
    for (size_t i = 0; i < items.size(); i++) { 
        // Search for item - Generated by Copilot
        std::string current_name = items[i]->getName();
        std::string lower_current_name = current_name;
        std::string lower_item_name = item_name;
        std::transform(lower_current_name.begin(), lower_current_name.end(), lower_current_name.begin(), ::tolower);
        std::transform(lower_item_name.begin(), lower_item_name.end(), lower_item_name.begin(), ::tolower);
        if (lower_current_name == lower_item_name) {
